...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, line):
        ...
        if cmd_name == "all":
            ...
            count = len([k for k in storage.all() if k.startswith(cls)])
            print(count)
        elif cmd_name in ("show", "destroy"):
            arg0 = args.strip().strip('"')
            getattr(self, f"do_{cmd_name}")(f"{cls} {arg0}")
        elif cmd_name == "update":
            # dict-style update: id, {key: val, ...}
            if args.strip().endswith('}'):
                id_part, dict_part = args.split(',', 1)
                id_clean = id_part.strip().strip('"')
                attr_dict = json.loads(dict_part.replace("'", '"'))
                for key, val in attr_dict.items():
                    self.do_update(f"{cls} {id_clean} {key} \"{val}\"")
            else:
                parts = [p.strip() for p in args.split(',', 2)]
                if len(parts) == 3:
                    id_clean = parts[0].strip().strip('"')
                    name = parts[1].strip().strip('"')
                    value = parts[2].strip().strip('"')
                    self.do_update(f"{cls} {id_clean} {name} \"{value}\"")
                else:
                    print("** invalid syntax **")
